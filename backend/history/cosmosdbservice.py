...
class CosmosConversationClient():
    ...
    async def delete_messages(self, conversation_id):
        ...
        if messages:
            for message in messages:
                resp = await self.container_client.delete_item(item=message['id'], partition_key=conversation_id)
                response_list.append(resp)
            return response_list


    async def get_conversations(self, limit, sort_order = 'DESC', offset = 0):
        query = f"SELECT * FROM c where c.type='conversation' order by c.updatedAt {sort_order}"
    ...
    async def get_messages(self, conversation_id):
        ...
        query = f"SELECT * FROM c WHERE c.conversationId = @conversationId AND c.type='message' ORDER BY c.timestamp ASC"
        messages = []
        async for item in self.container_client.query_items(query=query, parameters=parameters):
